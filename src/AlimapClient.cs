...
namespace Zongsoft.Externals.Alimap
{
	public class AlimapClient
	{
		...
		private const string GET_ID_URL = BASE_SEARCH_URL + "id";
		...
		#region 公共方法
		public async Task<IDictionary<string, object>> GetAsync(string tableId, string id)
		{
			if(string.IsNullOrEmpty(tableId))
				throw new ArgumentNullException(nameof(tableId));

			//构造请求消息
			var request = this.CreateRequest(HttpMethod.Get, GET_ID_URL, new SortedDictionary<string, string>
			{
				{ "key", _appId },
				{ "tableid", tableId },
				{ "_id", id },
		...
		public async Task<AlimapResult> CreateDataAsync(string tableId, IDictionary<string, object> data, DataMapping? mapping = null, CoordinateType coordinate = CoordinateType.GPS)
		{
			if(string.IsNullOrEmpty(tableId))
				throw new ArgumentNullException(nameof(tableId));

			//处理数据映射关系
			this.Mapping(data, coordinate, mapping);
		...
		public async Task<AlimapResult> UpdateDataAsync(string tableId, IDictionary<string, object> data, DataMapping? mapping = null, CoordinateType coordinate = CoordinateType.GPS)
		{
			if(string.IsNullOrEmpty(tableId))
				throw new ArgumentNullException(nameof(tableId));
		...
		#region 私有方法
		private void Mapping(IDictionary<string, object> data, CoordinateType coordinate, DataMapping? mapping)
		{
			...
			if(!string.IsNullOrEmpty(mapping.Value.Id) && data.TryGetValue(mapping.Value.Id, out value))
				data["_id"] = value;

			key = string.IsNullOrWhiteSpace(mapping.Value.Name) ? "Name" : mapping.Value.Name;
			if(data.TryGetValue(key, out value))
				data["_name"] = value;

			key = string.IsNullOrWhiteSpace(mapping.Value.Name) ? "Address" : mapping.Value.Name;
			if(data.TryGetValue(key, out value))
				data["_address"] = value;

			var keyX = string.IsNullOrWhiteSpace(mapping.Value.Longitude) ? "Longitude" : mapping.Value.Longitude;
			var keyY = string.IsNullOrWhiteSpace(mapping.Value.Latitude) ? "Latitude" : mapping.Value.Latitude;
